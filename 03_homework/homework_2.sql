...
-- Using the product table, write a query that outputs the product_id and product_name columns and add a column called prod_qty_type_condensed that displays the word “unit” if the 
...
SELECT 	product_id
		...
		,CASE product_qty_type
			WHEN 'unit' THEN 'unit'
			ELSE 'bulk' -- Else also include entries where product_qty_type is NULL
		END as prod_qty_type_condensed 
	from product
	

--We want to flag all of the different types of pepper products that are sold at the market. Add a column to the previous query called pepper_flag that 
...
SELECT v.*, vb.*  
FROM vendor v
INNER JOIN vendor_booth_assignments vb on
